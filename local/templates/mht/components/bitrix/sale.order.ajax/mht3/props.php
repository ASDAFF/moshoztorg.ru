<?
	...
	// тут объявляются showFilePropertyField и PrintPropsForm
		...
		<?
			...
			if(
				...
				if($arParams["ALLOW_NEW_PROFILE"] == "Y"){
					...
					echo getProfileIDSelectHTML($arResult["ORDER_PROP"]["USER_PROFILES"], GetMessage("SOA_TEMPL_PROP_NEW_PROFILE"));
				}
				else{
					...
					if(count($arResult["ORDER_PROP"]["USER_PROFILES"]) == 1){
						...
						echo getProfileIDSelectHTML($arResult["ORDER_PROP"]["USER_PROFILES"]);
					}
				}
			}
			else{
	               ...
	               <h4>
						...
						<?
							...
							if(
								...
								$name = GetMessage('SOA_TEMPL_BUYER_SHOW');
							}
							elseif(
								$bHideProps &&
								$_POST["showProps"] == "Y"
							){
								$name = GetMessage('SOA_TEMPL_BUYER_HIDE');
							}
