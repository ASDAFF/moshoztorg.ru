...
if ($_REQUEST["delivery"])
{
    ...
    $arDelivery = ExtraServices\Manager::getStoresFields($deliveryId, false);
    $arStoreId = $arDelivery['PARAMS']['STORES'];
	...
	while ($arStoreTmp = $dbList->Fetch())
	{
		if($arStoreTmp["GPS_N"]==0){
			$arStoreTmp["GPS_N"] = "55.75222";
		}
		if($arStoreTmp["GPS_S"]==0){
			$arStoreTmp["GPS_S"] = "37.61556";
		}
...
<tr>
	...
	<td class="ora-store">
		...
		foreach($arStore as $k=>$v){
			if(empty($v["GPS_N"]) || empty($v["GPS_S"])){
				unset($arStore[$k]);
			}
		}
		?>
				...
				<tr class="store_row <?=$checked?>" id="row_<?=$val["ID"]?>" onclick="setChangeStore(<?=$val["ID"]?>);">
					...
					<td class="<?=($countCount != $i)?"lilne":"last"?>">
						<label for="store_<?=$val["ID"]?>">
							<?if(!empty($val["TITLE"])){?><div class="name"><?=htmlspecialcharsbx($val["TITLE"])?></div><?}?>
							<?if(!empty($val["PHONE"])){?><div class="phone"><?=GetMessage('MAP_PHONE');?>: <?=htmlspecialcharsbx($val["PHONE"])?></div><?}?>
							<?if(!empty($val["EMAIL"])){?><div class="email"><?=GetMessage('MAP_EMAIL');?>: <a href="mailto:<?=htmlspecialcharsbx($val["EMAIL"])?>"><?=htmlspecialcharsbx($val["EMAIL"])?></a></div><?}?>
							<?if(!empty($val["ADDRESS"])){?><div class="adres"><?=GetMessage('MAP_ADRES');?>: <?=htmlspecialcharsbx($val["ADDRESS"])?></div><?}?>
							<?if(!empty($val["SCHEDULE"])){?><div class="shud"><?=GetMessage('MAP_WORK');?>: <?=htmlspecialcharsbx($val["SCHEDULE"])?></div><?}?>
						</label>
