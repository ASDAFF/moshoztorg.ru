<?
	...
	if(!function_exists("getProfileIDSelectHTML")){
		function getProfileIDSelectHTML($profiles, $emptyName = null){
			?>
				<select name="PROFILE_ID" id="ID_PROFILE_ID" onChange="SetContact(this.value)">
					<?
						if(is_string($emptyName)){
							?>
								<option value="0">
									<?=$emptyName?>
								</option>
						...
						foreach($profiles as $profile){
							?>
								<option value="<?= $profile["ID"] ?>"<?=$profile["CHECKED"]=="Y" ? " selected" : ""?>><?=$profile["NAME"]?></option>
							<?
						}
					?>
				</select>
