...
$(document).ready(function() {
	...
	$('[name="form1"] input[type="submit"]').on('click', function(){
		var form = $(this).parents('form');
		var pass1 = form.find('[name="NEW_PASSWORD"]').val();
		var pass2 = form.find('[name="NEW_PASSWORD_CONFIRM"]').val();

		if(pass1 != '' && pass1 != pass2) {
			form.find('[name="NEW_PASSWORD_CONFIRM"]').addClass('error');
			return false;
		}
	});

	$('[name="NEW_PASSWORD_CONFIRM"]').on('keyup keypress keydown', function(){
		var form = $(this).parents('form');
		var pass1 = form.find('[name="NEW_PASSWORD"]').val();
		var pass2 = form.find('[name="NEW_PASSWORD_CONFIRM"]').val();

		if(pass1 == pass2) {
			form.find('[name="NEW_PASSWORD_CONFIRM"]').removeClass('error');
		}
	});
