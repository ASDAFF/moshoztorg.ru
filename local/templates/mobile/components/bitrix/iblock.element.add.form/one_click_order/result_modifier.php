...
$arResult['PROPERTY_IDS'] = $arProperties;
...
foreach ($arParams["PROPERTY_CODES"] as $propertyID):
    ...
    ob_start();
    $sLabelClass = "";
    if ($sPropertyCode=="EVENT_DATE")
        $sLabelClass = 'class="datepickers"';
    ?><label for="field<?=$propertyID?>"  <?php echo $sLabelClass;?>>
        <p><?if (intval($propertyID) > 0):?>
    ...
    switch ($INPUT_TYPE):
        ...
        case "N":
            for ($i = 0; $i<$inputNum; $i++)
            {
                ...
                $inputClass = "";
                switch($sPropertyCode){
                    case "PHONE":
                        $inputClass = 'class="phone"';
                        break;
                    /*case "EVENT_DATE":
                        $inputClass = 'class="hasDatepicker"';
                        break;*/
                }

                $placeholder = "";
                if (array_key_exists($sPropertyCode,$arParams['PLACEHOLDERS'])):
                    $placeholder = 'placeholder="'.$arParams['PLACEHOLDERS'][$sPropertyCode].'"';
                endif?>
                <input  <?php echo  $inputClass;?>  <?php echo $placeholder;?> id="<?=$sPropertyCode?>" type="text"  name="PROPERTY[<?=$propertyID?>][<?=$i?>]" value="<?=$value?>" /><?
