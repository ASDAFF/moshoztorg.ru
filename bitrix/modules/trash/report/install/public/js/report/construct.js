...
function rebuildSortSelect()
{
	...
	for (i in columnList)
	{
		if (columnList[i].value != '')
		{
			...
			if (GLOBAL_BX_REPORT_USING_CHARTS)
			{
				...
				if (chartSelects)
				{
					for (var j in chartSelects)
					{
						...
						dataType = parseSelectColumnType(columnContainer);
						selectOption.setAttribute('data_type', dataType);
						validDataType = true;
						if (chartTypeSelect)
						{
							validDataType = false;
							chartType = chartTypeSelect.value;
							if (valueTypes[chartType]) validDataType = valueTypes[chartType].indexOf(dataType) >= 0;
						}
			...
			// check if grc
			var calc_select = BX.findChild(columnContainer, {tag:'select', 'className': 'reports-add-col-select-calc'}, true);
