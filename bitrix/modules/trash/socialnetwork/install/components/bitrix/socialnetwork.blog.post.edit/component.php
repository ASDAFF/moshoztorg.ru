...
)
{
	...
	if($arParams["USER_ID"] == $user_id)
		...
		$arResult["perms"] = CBlogPost::GetSocNetPostPerms($arPost["ID"], true, $user_id, $arPost["AUTHOR_ID"]);
				...
				if(strlen($arResult["ERROR_MESSAGE"]) <= 0)
				{
					...
					if(!$bError)
					{
						...
						else
						{
							...
							$dbDuplPost = CBlogPost::GetList(array("ID" => "DESC"), array("BLOG_ID" => $arBlog["ID"]), false, array("nTopCount" => 1), array("ID", "BLOG_ID", "AUTHOR_ID", "DETAIL_TEXT"));
							if($arDuplPost = $dbDuplPost->Fetch())
							{
								if($arDuplPost["BLOG_ID"] == $arFields["BLOG_ID"] && IntVal($arDuplPost["AUTHOR_ID"]) == IntVal($arFields["AUTHOR_ID"]) && md5($arDuplPost["DETAIL_TEXT"]) == md5($arFields["DETAIL_TEXT"]))
								{
									$bError = true;
