...
if (CModule::IncludeModule("socialnetwork"))
{
	if ($GLOBALS["USER"]->IsAuthorized())
	{
		...
		if (
			strlen($arParams["ex"]) > 0 
			&& $arParams["ex"] == "E" 
			&& strlen($arParams["site"]) > 0 
		...
		elseif (
			strlen($arParams["ex"]) > 0 
			&& $arParams["ex"] == "EA" 
			&& $GLOBALS["APPLICATION"]->GetGroupRight("socialnetwork", false, "Y", "Y", array($arParams["site"], false)) >= "K" 
			&& CModule::IncludeModule('extranet')
		)
		{
		...
		elseif (
			CModule::IncludeModule('extranet') 
			&& CExtranet::IsIntranetUser($arParams["site"])
		)
