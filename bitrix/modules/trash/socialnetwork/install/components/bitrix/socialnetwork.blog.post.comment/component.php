	...
	if($arResult["Perm"]>=BLOG_PERMS_READ)
	{
		...
		if(is_array($arResult["CommentsResult"]))
		{
			...
			foreach($arResult["CommentsResult"] as $k1 => $v1)
			{
				...
				if(!empty($arParams["POST_DATA"]["SPERM_HIDDEN"]) && strlen($v1["SHARE_DEST"]) > 0)
				{
					...
					if(!empty($dest))
					{
						...
						if($bDestCuted)
						{
								...
								if(!in_array($destId, $arParams["POST_DATA"]["SPERM_HIDDEN"]))
								{
									...
									if($destId == "UA")
									{
										if (!$arParams["POST_DATA"]["bExtranetSite"] && defined("BITRIX24_PATH_COMPANY_STRUCTURE_VISUAL"))
											$arDest[] = "[url=".BITRIX24_PATH_COMPANY_STRUCTURE_VISUAL."]".(IsModuleInstalled("intranet") ? GetMessage("B_B_SHARE_ALL") : GetMessage("B_B_SHARE_ALL_BUS"))."[/url]";
										else
											$arDest[] = (IsModuleInstalled("intranet") ? GetMessage("B_B_SHARE_ALL") : GetMessage("B_B_SHARE_ALL_BUS"));
											;
							...
							$arDest[] = GetMessage("B_B_SHARE_HIDDEN");
				...
				if((IsModuleInstalled("intranet") && $bAuthor) || ($arResult["Perm"] >= BLOG_PERMS_FULL && !IsModuleInstalled("intranet")) || CSocNetUser::IsCurrentUserModuleAdmin() || $APPLICATION->GetGroupRight("blog") >= "W")
				{
