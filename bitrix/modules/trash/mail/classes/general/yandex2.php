...
global $MESS;
include GetLangFileName($_SERVER['DOCUMENT_ROOT'].BX_ROOT.'/modules/mail/lang/', '/classes/general/yandex2.php');
...
class CMailYandex2
{
	...
	/**
	 * http://pddimp.yandex.ru/api2/admin/email/edit
	 *
	 * unknown - временный сбой, либо не опознанная ошибка на нашей стороне (попробуйте еще раз или напишите в службу поддержки)
	 * no_token, no_domain, no_ip - отсутсвуют обязательные параметры
	 * bad_domain - пустое или не соответствующее RFC доменное имя
	...
	{
		$result = self::post('https://pddimp.yandex.ru/api2/admin/email/edit', array(
			'token'  => $token,
			'domain' => $domain,
			'login'  => $login
		));
