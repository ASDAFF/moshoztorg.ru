...
class CLearnRenderAdminUnilessonList
{
	...
	public function BuildList()
	{
		...
		while ($arRes = $this->rsData->NavNext(false))	// NavNext: don't extract fields to globals
		{
			...
			{
				$hrefPrefix = 'learn_unilesson_edit.php?LESSON_ID=' . $arRes['LESSON_ID'] 
			...
			if ( ! $this->IsSearchMode() )
				$row->AddViewField('NAME', '<span class="adm-list-table-icon-link"><span class="adm-submenu-item-link-icon adm-list-table-icon '.$icon.'"></span><span class="adm-list-table-link">'.htmlspecialcharsbx($arRes['NAME'])."</span></span>");
			else
			{
			...
			if ( ! $this->IsSearchMode() )
			{
				...
				*/
				...
				if ($isEnoughRightsForDisbandLesson)
				{
					...
					if ($parentPathesCnt >= 1)
					{
						$actionDisband = "if(confirm('" 
							. str_replace(
								'#CNT#', 
								$parentPathesCnt, 
								CUtil::JSEscape(GetMessage('LEARNING_CONFIRM_DISBAND_LESSON_WITH_PARENT_PATHES'))
							) 
							. "')) "
							. '{ ' . $actionDisband . ' }';
					}
					...
					$actionDisband = "if(confirm('" 
						. CUtil::JSEscape(GetMessage('LEARNING_ADMIN_MENU_DISBAND_QUESTION'))
						. "')) " 
						. '{ ' . $actionDisband . ' }';
					...
					if ($parentPathesCnt >= 1)
					{
						...
						$action = "if(confirm('" 
							. str_replace(
								'#CNT#', 
								$parentPathesCnt, 
								GetMessage("LEARNING_CONFIRM_DEL_LESSON_WITH_PARENT_PATHES")
							) 
							. "')) " 
							. $action;
					}
					...
					{
						$action = "if(confirm('" . GetMessage('LEARNING_CONFIRM_DEL_MESSAGE') . "')) " 
							. $action;
					}
				}
