...
use Bitrix\Sale\Location\Import\CSVReader;
use Bitrix\Sale\Location\Import\Process;
...
final class ImportProcess extends Process
{
	...
	protected function locationCodeExists($code)
	{
		if(!strlen($code))
			return false;

		$code = $this->dbHelper->forSql($code);

		$res = $this->dbConnection->query('select ID from '.Location\LocationTable::getTableName().' where CODE = '.$code)->fetch();
	...
	protected function importBlock(&$block)
	{
		...
		foreach($block as $i => $data)
		{
			...
			{
				$existedId = $this->locationCodeExists($code);
			...
			$data['LONGITUDE'] = intval($data['LONGITUDE']);
			$data['LATITUDE'] = intval($data['LATITUDE']);
	...
	public function deleteAll()
	{
		...
		//$this->dbConnection->query('delete from '.Location\ExternalServiceTable::getTableName());
		$this->dbConnection->query('truncate table '.Location\ExternalTable::getTableName());

		// delete groups
		$this->dbConnection->query('truncate table '.Location\GroupTable::getTableName());

		// delete group names
		$this->dbConnection->query('truncate table '.Location\Name\GroupTable::getTableName());

		// delete links with sites
		Location\SiteLocationTable::deleteAll();

		// delete types
		//if($this->checkSource(self::SOURCE_REMOTE))
		//	$this->dbConnection->query('delete from '.Location\TypeTable::getTableName());
	...
	protected function buildExternalSerivceTable()
	{
		...
		foreach($external as $line)
		{
			...
			{
				$res = Location\ExternalServiceTable::add($line);
				if(!$res->isSuccess())
