	...
	protected function processOrderAcceptRequest($arPostData)
	{
		...
		if($orderId <= 0)
		{
			...
			if(!$res->isSuccess())
			{
				...
				$result['order']['test'] = 'Cant create order';
			...
			if(!$this->checkBasketPrice($arPostData['order']['items'], $order))
			{
				...
				else
				{
					...
					$result['order']['test'] = 'CheckBasketPrice fail';
			...
			if(!$res->isSuccess())
			{
				...
				$result['order']['test'] = 'Cant save order';
			...
			if(!$res->isSuccess())
			{
				...
				$result['order']['test'] = 'cant add trading platform';
