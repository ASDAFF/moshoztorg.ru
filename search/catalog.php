...
//dm( $arResult['results']['filters'] );
                ...
                <div class="search_results">
                    <div class="search_result_list">
                        ...
                        foreach ($arResult['results']['filters'] as $kf => $arFilter) {
                            ...
                            //if ( $arFilter['name'] == 'Страна производитель' ) $arFilter['name'] = 'country';

                            if ($arFilter['title'] == 'vendor') {
                                $arFilter['title'] = "Бренды";
                            }
            ...
            <li class="showhidden"><a href="javascript:void(0)">Показать скрытые (<?=count($arFilter['values'])-VISIBLE_ITEMS_COUNT?>)</a></li>
            <li class="hidehidden" style="display: none"><a href="javascript:void(0)">Минимизировать список</a></li><?endif?>
            </ul><?
        }
        else
        {
            ...
            <ul class="search-found-sections filter">
                ...
                foreach ($arFilter['values'] as $kv => $arValue) {
                    ...
                    <div class="search-found-section-title filteritems">
                        <label>
                            <input type="checkbox" name="<?= str_replace(' ','_',$arFilter['name']) ?>" value="<?= str_replace(' ','+',$arValue['value']) ?>"
                                   style="display: none">
                            <span class="catname"><?= $arValue['value'] ?> [<?=$arValue['count']?>]</span>
                        </label>
                    </div>
                    </a></li>
